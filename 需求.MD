...
| `--schema-table-regex` | []string | N | schema.table 范围表达式（不是正则表达式），例 `db_[0-3].my_table_[0-9]` |
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| (无) | - | - | 无额外参数，使用全局参数和交互式操作，支持断点续看 |

**断点续看功能**：
- 自动保存浏览位置：每次用户按 `q` 退出时，自动保存当前位置到 `~/.binlogx/checkpoints/`
- 自动恢复断点：下次运行相同数据源的 parse 命令时，自动检测并提示是否继续
- 支持模式：
  - 离线文件模式（`--source`）：完全支持
  - 在线 MySQL 模式（`--db-connection`）：完全支持
  - 混合模式：当同时指定 `--source` 和 `--db-connection` 时，优先使用 `--source` 且支持断点续看
...
### 6.1 断点续看（Checkpoint/Breakpoint Resume）
- **保存位置**：`~/.binlogx/checkpoints/` 目录
- **适用命令**：`parse`（交互式浏览）
- **保存触发**：用户按 `q` 退出时自动保存当前浏览位置和事件信息
- **恢复机制**：下次运行相同数据源的 parse 命令时自动检测，提示用户选择继续或重新开始
- **支持的数据源**：
  - 离线文件模式（`--source`）：基于文件路径的哈希值作为标识
  - 在线 MySQL 模式（`--db-connection`）：基于连接字符串的哈希值作为标识
- **保存内容**：文件名、位置（logPos）、时间戳、事件类型、数据库、表名

### 6.2 列名缓存（Table Meta Cache）
